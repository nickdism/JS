...
let personalCount = prompt('Какая размерность поля?', )
const gameField = [];
let clickCounter = 0;
...
function cellClickHandler (row, col) {
    if (gameField[row][col]===EMPTY) {
        let fieldState = clickCounter % 2 === 0 ? CROSS : clickRandomZero(gameField);
...
function addResetListener () {
    ...
    resetButton.addEventListener('click', resetClickHandler); // не понимаем, почему не работает кнопка сначала
}

function resetClickHandler () {
    cleanGameField(personalCount, gameField);
    console.log('reset!');
}

function cleanGameField(dimension, gameField) {
    for (let i = 0; i < dimension; i++) {
        for (let j = 0; j < dimension; j++) {
            gameField[i][j] = EMPTY;
        }
